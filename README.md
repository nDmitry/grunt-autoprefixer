...
If `true`, the plugin will try to find an existing source map (generated by a pre-processor, for instance) file and generate a new map based on a found map (or just generate a new map). If string is specified, the plugin will look for an input source map at the specified directory.
